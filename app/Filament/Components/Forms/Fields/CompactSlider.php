...
class CompactSlider extends Field implements Contracts\HasNestedRecursiveValidationRules
{
     ...
     * @var view-string
     */
    protected string $view = 'filament.components.compact-slider';
    ...
    protected function setUp(): void
    {
        ...
        $this->default(static fn (CompactSlider $component): float | int => $component->getMinValue());

        $this->required();

        $this->rule('numeric', static fn (CompactSlider $component): bool => ! $component->isMultiple());

        $this->rule(static function (CompactSlider $component): string {
            $value = $component->getMinValueWithPadding();

            return "min:{$value}";
        }, static fn (CompactSlider $component): bool => ! $component->isMultiple());
        ...
        }, static fn (CompactSlider $component): bool => (! $component->isMultiple()) && filled($component->getStep()));

        $this->rule('array', static fn (CompactSlider $component): bool => $component->isMultiple());

        $this->nestedRecursiveRule('numeric', static fn (CompactSlider $component): bool => $component->isMultiple());

        $this->nestedRecursiveRule(static function (CompactSlider $component): string {
            $value = $component->getMinValueWithPadding();
    ...
    public function getDefaultStateCasts(): array
    {
        return [
            ...
            new SliderStateCast($this->getDecimalPlaces()),
        ];
    }
